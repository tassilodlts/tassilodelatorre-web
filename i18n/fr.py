# -*- coding: utf-8 -*-
T = {
# ---------- metas ----------
"Tassilo de la Torre, Adventure Photographer · Model · Architect · Madrid":
    "Tassilo de la Torre, photographe d'aventure · mannequin · architecte · Madrid",
"Tassilo de la Torre - Adventure photographer, model and architect":
    "Tassilo de la Torre, photographe d'aventure, mannequin et architecte",

# ---------- en-tête / hero ----------
"Skip to content": "Aller au contenu",
"Setting off 4 September": "Départ le 4 septembre",
"Partner slots open →": "Places de partenaire encore libres →",
"Work": "Travaux",
"Expeditions": "Expéditions",
"Writing": "Textes",
"Prints": "Tirages",
"Partners": "Marques",
"Contact": "Contact",
"Adventure photographer · Model · Architect · Madrid":
    "Photographe d'aventure · mannequin · architecte · Madrid",
"Partner with the 2026 expeditions": "Devenir partenaire des expéditions 2026",
"Follow the expedition on Instagram · @tassilodlts":
    "Suivre l'expédition sur Instagram · @tassilodlts",
"Selected work": "Travaux choisis",
"Currently": "En ce moment",
"on board": "à bord",
"· the brands backing the 2026 expeditions":
    "· les marques qui soutiennent les expéditions 2026",

# ---------- travaux ----------
"The Danube beneath the Walhalla. EUROPA, fully loaded, Fluffo at the bow.":
    "Le Danube sous le Walhalla. EUROPA, chargé à bloc, Fluffo à la proue.",
"The river, straight down, by drone. Paddling the Tiber, Italy.":
    "La rivière, à la verticale, au drone. Descente du Tibre, Italie.",
"Dawn on Monte Molare, the Bay of Naples and Vesuvius behind.":
    "Aube sur le Monte Molare, la baie de Naples et le Vésuve au fond.",
"Dawn inside Nuka Sakdari, Fluffo still asleep.":
    "Aube à l'intérieur de Nuka Sakdari, Fluffo dort encore.",
"The boat gets her name.": "Le bateau reçoit son nom.",
"A long night in an abandoned Armenian house. Türkiye.":
    "Une longue nuit dans une maison arménienne abandonnée. Turquie.",
"More from the road": "Encore de la route",
"Frames from the expeditions.": "Images des expéditions.",
"Rivers and ruins, camps and coastlines, and Fluffo, who never misses a launch.":
    "Des fleuves et des ruines, des bivouacs et des côtes, et Fluffo, qui ne rate jamais une mise à l'eau.",
"A bed among the columns · Lycian tomb, Türkiye":
    "Un lit entre les colonnes · tombeau lycien, Turquie",
"Golden hour with Fluffo · Türkiye": "Heure dorée avec Fluffo · Turquie",
"The temple still stands · Türkiye": "Le temple tient encore debout · Turquie",
"Slung above the river · Regensburg, Germany":
    "Suspendu au-dessus du fleuve · Ratisbonne, Allemagne",
"The stranded giant · Egypt": "Le géant échoué · Égypte",
"Under the painted vault · Romania": "Sous la voûte peinte · Roumanie",
"Leap into the shallows · Hungary": "Saut dans les basses eaux · Hongrie",
"Terraces of the Wachau · Austria": "Terrasses de la Wachau · Autriche",
"Beached at dusk · Croatia": "Échoué au crépuscule · Croatie",
"Camp on the gravel bar · Serbia": "Bivouac sur le banc de galets · Serbie",

# ---------- architecture ----------
"The architect's eye": "L'œil de l'architecte",
"Architecture, the other subject.": "L'architecture, l'autre sujet.",
"La Vela di Calatrava, Rome, by drone at dusk.":
    "La Vela di Calatrava, Rome, au drone à la tombée du jour.",
"Ribs, light, and a puddle for a mirror.":
    "Des nervures, de la lumière, et une flaque en guise de miroir.",
"Stone, symmetry, still water.": "Pierre, symétrie, eau immobile.",
"A tower holding on to the cliff. Find the red hat.":
    "Une tour accrochée à la falaise. Cherchez le bonnet rouge.",
"Scroll →": "Faire défiler →",
"See the full architecture work at Studio Kotorre →":
    "Voir tout le travail d'architecture chez Studio Kotorre →",

# ---------- expéditions ----------
"Expeditions 2026": "Expéditions 2026",
"Two journeys. One camera.": "Deux voyages. Un seul appareil.",
"Spain, under human power": "L'Espagne, à la seule force du corps",
"On the trail now · Two months": "En route · deux mois",
"A lap of the Balearics, filmed": "Un tour des Baléares, filmé",
"September · ~250 nautical miles": "Septembre · environ 250 milles nautiques",
"Around Mallorca and Menorca by sea kayak in stages, with Fluffo as crew, and I'll bring home a short documentary, daily films and a real body of photography.":
    "Faire le tour de Majorque et de Minorque en kayak de mer, par étapes, avec Fluffo comme équipage, et en rapporter un court documentaire, des vidéos quotidiennes et un vrai corpus photographique.",
"Want your gear on this journey? Partner with the 2026 expeditions →":
    "Envie que votre matériel soit du voyage ? Devenez partenaire des expéditions 2026 →",

# ---------- atlas ----------
"The Atlas": "L'Atlas",
"All the routes, on one globe.": "Tous les itinéraires, sur un seul globe.",
"The real routes, by bike, kayak, boat and thumb, from the Camino to Kyrgyzstan. Spin the globe, zoom in, follow the lines across the world.":
    "Les vrais itinéraires, à vélo, en kayak, en bateau et en stop, du Camino au Kirghizistan. Faites tourner le globe, zoomez et suivez les lignes à travers le monde.",
"Every line here was travelled under my own power and recorded on the way. I am building this into something anyone can use: a proper satellite and terrain atlas where you plot your own journeys and plan the next one, the tool that went missing when FATMAP was shut down. If you run an expedition and want your own globe,":
    "Chaque ligne d'ici, je l'ai parcourue par mes propres moyens et enregistrée en chemin. J'en fais quelque chose que tout le monde pourra utiliser : un vrai atlas satellite et relief où l'on trace ses propres voyages et où l'on prépare le suivant, l'outil qui manque depuis la fermeture de FATMAP. Si vous organisez une expédition et que vous voulez votre propre globe,",
"write to me": "écrivez-moi",
"By transport": "Par moyen de transport",
"Boat / ferry": "Bateau / ferry",
"Hitchhiking": "En stop",
"On foot": "À pied",
"Planned · Sept. 2026": "Prévu · sept. 2026",
"Dreaming": "En rêve",
"Drag to spin · scroll to zoom · Shift + drag to tilt":
    "Glissez pour tourner · molette pour zoomer · Maj et glisser pour incliner",
"See the September route": "Voir l'itinéraire de septembre",

# ---------- à propos ----------
"About": "À propos",
"Behind the camera.": "Derrière l'appareil.",
"The other kind of shoot.": "L'autre genre de séance.",
"I've had folding boats since I was twelve. Last summer I paddled a 1950s folding kayak 2,580 km down the Danube, from Ulm to the Black Sea, with my dog Fluffo as crew. I trained as an architect, I work as a model, and I've spent years in product photography. This year I'm crossing Spain on foot and by kayak, and paddling around Mallorca and Menorca.":
    "J'ai des bateaux pliants depuis mes douze ans. L'été dernier, j'ai descendu 2 580 km du Danube dans un kayak pliant des années cinquante, d'Ulm à la mer Noire, avec mon chien Fluffo comme équipage. J'ai une formation d'architecte, je travaille comme mannequin et j'ai passé des années en photographie de produit. Cette année, je traverse l'Espagne à pied et en kayak, et je fais le tour de Majorque et de Minorque à la pagaie.",
"Five languages, and a lifetime racing and paddling the coasts of southern Spain, Corsica and the Mediterranean.":
    "Cinq langues, et toute une vie de régates et de pagaie le long des côtes du sud de l'Espagne, de la Corse et de la Méditerranée.",
"down the Danube": "de Danube",
"Age 12": "À 12 ans",
"first folding boat": "premier bateau pliant",
"5 languages": "5 langues",
"spoken": "parlées",
"B.A. Architecture": "B.A. Architecture",
"Fluffo, first mate, regulation life vest. Corsica.":
    "Fluffo, second à bord, gilet réglementaire. Corse.",
"Winter": "Hiver",
"The season the kayak stays folded. Ski teacher in Garmisch-Partenkirchen, Germany.":
    "La saison où le kayak reste plié. Moniteur de ski à Garmisch-Partenkirchen, Allemagne.",
"End of the season, carried off the hill. Ski teacher in Schönried, Switzerland.":
    "Fin de saison, porté au bas de la piste. Moniteur de ski à Schönried, Suisse.",

# ---------- matériel ----------
"What I shoot with": "Avec quoi je photographie",
"The kit that comes along.": "Le matériel qui fait le voyage.",
"My main camera, weather-sealed and built to keep working when the conditions do not.":
    "Mon boîtier principal, tropicalisé et fait pour continuer quand les conditions, elles, s'arrêtent.",
"The everyday zoom, wide to short tele, for camp, coast and the people I meet.":
    "Le zoom de tous les jours, du grand-angle au petit télé, pour le bivouac, la côte et les gens que je croise.",
"The reach: wildlife, far shorelines and the small details across the water.":
    "L'allonge : la faune, les rives lointaines et les petits détails de l'autre côté de l'eau.",
"The view from the air, small enough to launch from a kayak deck.":
    "La vue d'en haut, assez petit pour décoller du pont d'un kayak.",
"In the water, on the hull, wherever a bigger camera cannot go.":
    "Dans l'eau, sur la coque, partout où un boîtier plus gros ne passe pas.",
"Plus a few smaller cameras for the moments the others miss.":
    "Et quelques petites caméras de plus pour les moments qui échappent aux autres.",

# ---------- textes ----------
"What I bring back besides pictures.": "Ce que je rapporte en plus des images.",
"Two pieces from the rivers. One is an essay about a boat and a continent. The other is the only continuous report of the navigable Tiber that exists, written so the next person has something I did not.":
    "Deux textes venus des fleuves. L'un est un essai sur un bateau et un continent. L'autre est le seul compte rendu continu du Tibre navigable qui existe, écrit pour que le suivant ait ce que je n'ai pas eu.",
"Essay · The Danube": "Essai · le Danube",
"On the roof of a Soviet boathouse in Budapest, patching a seventy year old folding kayak named Europa, the Parliament shining on the far bank. A thousand kilometres down, sixty days to the Black Sea, and a continent that needs repairing.":
    "Sur le toit d'un hangar à bateaux soviétique à Budapest, en train de rapiécer un kayak pliant de soixante-dix ans baptisé Europa, le Parlement illuminé sur l'autre rive. Mille kilomètres dans le dos, soixante jours jusqu'à la mer Noire, et un continent à réparer.",
"Read in English  ·": "Lire en anglais  ·",
"Auch auf Deutsch": "Aussi en allemand",
"Report · Italy · Deutsch": "Compte rendu · Italie · allemand",
"380 kilometres alone by canoe from Sansepolcro to the sea, in a boat bought from an 88 year old woman. Every weir, portage, rapid and campsite, day by day, with the annotated aerial maps. Written because no complete report of this river existed.":
    "380 kilomètres en solitaire en canoë, de Sansepolcro à la mer, dans un bateau acheté à une dame de 88 ans. Chaque barrage, chaque portage, chaque rapide et chaque bivouac, jour après jour, avec les vues aériennes annotées. Écrit parce qu'il n'existait aucun compte rendu complet de ce fleuve.",
"Read the report →": "Lire le compte rendu →",
"All writing →": "Tous les textes →",

# ---------- presse ----------
"Commissioned by": "Commandes de",
"Photography and route reports from the 2026 expeditions. Motor Presse Stuttgart.":
    "Photographies et récits d'itinéraire des expéditions 2026. Motor Presse Stuttgart.",
"Text and photography from the Spanish stages, on commission.":
    "Texte et photographies des étapes espagnoles, sur commande.",

# ---------- marques ----------
"For brands &amp; partners": "Pour les marques et les partenaires",
"One person who is photographer, model and location at once.":
    "Une seule personne qui est à la fois photographe, mannequin et décor.",
"Film": "Vidéo",
"A documentary of twenty to forty minutes, plus daily short films you are free to run on your own channels.":
    "Un documentaire de vingt à quarante minutes, plus des vidéos courtes quotidiennes que vous êtes libres de diffuser sur vos propres canaux.",
"Photography": "Photographie",
"Fifty to a hundred finished photographs, with full commercial rights.":
    "De cinquante à cent photographies finies, avec tous les droits commerciaux.",
"Proof": "La preuve",
"\"Every collaboration is a first step, and I'd love to take the good ones a great deal further.\"":
    "« Chaque collaboration est un premier pas, et les bonnes, j'aimerais beaucoup les mener bien plus loin. »",
"And every collaboration plants ten trees.": "Et chaque collaboration plante dix arbres.",
"I dig the holes myself, in the emptiest highlands in Spain, and you get the coordinates.":
    "Les trous, je les creuse moi-même, dans les hauts plateaux les plus vides d'Espagne, et vous recevez les coordonnées.",
"Write to me": "Écrivez-moi",
"Download media kit (PDF)": "Télécharger le media kit (PDF)",
"Casting or an agency?": "Casting ou agence ?",
"Model digitals on request.": "Digitals de mannequin sur demande.",

# ---------- tirages ----------
"Prints &amp; editions": "Tirages et éditions",
"Prints for the wall.": "Des tirages pour le mur.",
"Fine-art prints of the expeditions, from the water and from the air. Archival paper, signed and numbered, shipped worldwide.":
    "Tirages fine art des expéditions, depuis l'eau et depuis les airs. Papier de conservation, signés et numérotés, expédiés dans le monde entier.",
"The Danube beneath the Walhalla": "Le Danube sous le Walhalla",
"La Vela di Calatrava, Rome": "La Vela di Calatrava, Rome",
"The cliff church of Artvin": "L'église accrochée à la falaise d'Artvin",
"Dawn on Monte Molare, Naples": "Aube sur le Monte Molare, Naples",
"Every print plants a tree.": "Chaque tirage plante un arbre.",
"One native tree, planted by hand in Guadalajara for every photograph sold. Not a donation: my own shovel, and the coordinates published.":
    "Un arbre du pays, planté à la main à Guadalajara pour chaque photographie vendue. Pas un don : ma propre pelle, et les coordonnées publiées.",
"Archival cotton paper, signed and numbered, shipped worldwide. Pick a size above and tell me which photograph you want. If you would rather have a size that is not listed, ask.":
    "Papier coton de conservation, signés et numérotés, expédiés dans le monde entier. Choisissez un format ci-dessus et dites-moi quelle photographie vous voulez. Si vous préférez un format qui n'est pas dans la liste, demandez.",

# ---------- nuka sakdari ----------
"Nuka Sakdari, on its shelf of rock above the valley. Artvin, Türkiye.":
    "Nuka Sakdari, sur sa vire rocheuse au-dessus de la vallée. Artvin, Turquie.",
"A project to support": "Un projet à soutenir",
"Eleven hundred years old, and running out of time.":
    "Mille cent ans d'âge, et le temps lui manque.",
"High above a roadless valley in Artvin stands":
    "Bien au-dessus d'une vallée sans route, à Artvin, se dresse",
", a ninth-century Georgian basilica of the Tao-Klarjeti school: its stone roof open to the sky, its walls deteriorating, never restored and scarcely documented. With a small team of architects I am making the first complete architectural record in its history, and using film to fund its emergency stabilisation. We fund, document, advocate and film, always under the authority of the Ministry of Culture. We do not touch the stones.":
    ", une basilique géorgienne du IXe siècle de l'école de Tao-Klardjétie : sa toiture de pierre ouverte au ciel, ses murs qui se dégradent, jamais restaurée et à peine documentée. Avec une petite équipe d'architectes, je réalise le premier relevé architectural complet de son histoire, et je me sers du film pour financer sa consolidation d'urgence. Nous finançons, nous documentons, nous plaidons et nous filmons, toujours sous l'autorité du ministère de la Culture. Nous ne touchons pas aux pierres.",
"Ways to help": "Comment aider",
"Gear": "Matériel",
"Cameras, a drone, and access and documentation kit for the survey.":
    "Des appareils, un drone, et du matériel d'accès et de relevé pour le chantier.",
"Funding": "Financement",
"To pay for the architectural survey and the emergency stabilisation.":
    "Pour payer le relevé architectural et la consolidation d'urgence.",
"Contacts": "Contacts",
"Heritage bodies, foundations, film funds, Turkish authorities.":
    "Organismes du patrimoine, fondations, fonds cinéma, autorités turques.",
"Experience": "Expérience",
"Conservation architects, engineers, photogrammetry, Georgian-heritage experts, film.":
    "Architectes du patrimoine, ingénieurs, photogrammétrie, spécialistes du patrimoine géorgien, cinéma.",
"Support the project": "Soutenir le projet",
"See the full proposal →": "Voir le dossier complet →",

# ---------- newsletter / pied ----------
"The dispatch": "Le bulletin",
"One email when a route is finished.": "Un e-mail quand un itinéraire est terminé.",
"No newsletter, no schedule. A note when a new expedition report goes up, when the Atlas gains a line, or when new prints are ready. That is all.":
    "Pas de newsletter, pas de calendrier. Un mot quand un nouveau récit d'expédition est en ligne, quand l'Atlas gagne une ligne, ou quand de nouveaux tirages sont prêts. C'est tout.",
"Put me on the list": "Inscrivez-moi",
"Under human power.": "À la seule force du corps.",

# ---------- attributs ----------
"Toggle menu": "Ouvrir et fermer le menu",
"Follow Tassilo on Instagram, @tassilodlts": "Suivre Tassilo sur Instagram, @tassilodlts",
"Brands currently on board": "Marques actuellement à bord",
"Architecture photographs, scroll horizontally": "Photographies d'architecture, faire défiler horizontalement",
"Interactive satellite globe of Tassilo's journeys": "Globe satellite interactif des voyages de Tassilo",

# ---------- contador (JavaScript) ----------
"Setting off in {n} days · Mallorca and Menorca by sea kayak":
    "Départ dans {n} jours · Majorque et Minorque en kayak de mer",
"Setting off tomorrow · Mallorca and Menorca by sea kayak":
    "Départ demain · Majorque et Minorque en kayak de mer",
"Setting off today · Mallorca and Menorca by sea kayak":
    "Départ aujourd'hui · Majorque et Minorque en kayak de mer",
"On the water · Mallorca and Menorca by sea kayak":
    "Sur l'eau · Majorque et Minorque en kayak de mer",

# ---------- titular y textos nuevos ----------
"Small boats, long journeys,":
    "Des petits bateaux, de longs voyages",
"and a lot of world to look at.":
    "et beaucoup de monde à regarder.",
"Small boats, long journeys, and a lot of world to look at. Spain and the Balearics, 2026.":
    "Des petits bateaux, de longs voyages, et beaucoup de monde à regarder. Espagne et Baléares, 2026.",
"Where these come from.":
    "D'où viennent ces images.",
"I studied architecture (B.A., TU München), so I tend to look at ruins, structures and landscapes the way I used to draw buildings. Most of these were taken from the water or from the air.":
    "J'ai étudié l'architecture (B.A., TU München), alors j'ai tendance à regarder les ruines, les structures et les paysages comme je dessinais autrefois des bâtiments. La plupart ont été prises depuis l'eau ou depuis les airs.",
"Light enough to carry for months, and able to take salt, sand and rain. This is the whole list, nothing extra.":
    "Assez léger pour être porté des mois, et capable d'encaisser le sel, le sable et la pluie. C'est toute la liste, il n'y a rien de plus.",
"Your product after months of real use, in salt, rain and sun.":
    "Votre produit après des mois d'usage réel, dans le sel, la pluie et le soleil.",

# ---------- correcciones de agosto ----------
". By sea kayak, on foot, by bike and by thumb, usually with my dog Fluffo.":
    ". En kayak de mer, à pied, à vélo et en stop, le plus souvent avec mon chien Fluffo.",
"Tassilo de la Torre, adventure photographer, model and architect based in Madrid. Photographs from long journeys by sea kayak, on foot, by bike and hitchhiking, usually with my dog Fluffo.":
    "Tassilo de la Torre, photographe d'aventure, mannequin et architecte installé à Madrid. Des photographies de longs voyages en kayak de mer, à pied, à vélo et en stop, le plus souvent avec mon chien Fluffo.",
"These come from journeys by sea kayak, on foot, by bike and hitchhiking. I sleep outside most nights, in caves, in ruins, on gravel bars and on summits, so I am usually there early and late, when the light is good. A trip runs for weeks or months.":
    "Elles viennent de voyages en kayak de mer, à pied, à vélo et en stop. La plupart des nuits, je dors dehors, dans des grottes, dans des ruines, sur des bancs de galets et sur des sommets, donc je suis en général là tôt et tard, quand la lumière est bonne. Un voyage dure des semaines ou des mois.",
"Across Spain on foot and by sea kayak: the Almería desert, the plains of Castilla, the wet green north coast. Sleeping outside most nights, with Fluffo.":
    "Traverser l'Espagne à pied et en kayak de mer : le désert d'Almería, les plaines de Castille, la côte nord verte et mouillée. La plupart des nuits dehors, avec Fluffo.",
}
